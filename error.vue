<template>
  <div class="error-page column flex-center cosmic-bg">
    <div v-for="i in asteroidCount" :key="i" class="asteroid" :style="asteroidStyle(i)"></div>
    ...
    <q-btn to="/" label="Эвакуироваться на главную" text-color="white" color="green" size="lg"
      class="shield-btn q-px-xl q-mb-lg" no-caps />
  </div>
</template>
...
.asteroid {
  ...
  &:nth-child(7n + 3) {
    clip-path: polygon(50% 0%, 70% 20%, 100% 30%,
        80% 50%, 100% 70%, 70% 80%,
        50% 100%, 30% 80%, 0% 70%,
        20% 50%, 0% 30%, 30% 20%);
  }
...
@keyframes fall {
  to {
    transform:
      translateY(calc(100vh + 100px)) rotate(var(--rotation));
  }
}
